...
function isUrlInput(text) {
    ...
    // 常见视频网站URL模式检测
    try {
        // 先尝试直接 new URL，能成功说明是完整URL
        if (/^https?:\/\//i.test(text)) {
            new URL(text);
            return true;
        }
        // 匹配无协议前缀的域名格式 (如 v.qq.com/xxx)
...
function updateSearchButton() {
    ...
    if (isUrlInput(value)) {
        btn.textContent = '解析';
        btn.setAttribute('aria-label', '解析按钮');
        btn.classList.remove('bg-white', 'text-black', 'hover:bg-gray-200');
        btn.classList.add('bg-gradient-to-r', 'from-indigo-500', 'to-purple-500', 'text-white', 'hover:from-indigo-600', 'hover:to-purple-600');
    } else {
        btn.textContent = '搜索';
        btn.setAttribute('aria-label', '搜索按钮');
        btn.classList.remove('bg-gradient-to-r', 'from-indigo-500', 'to-purple-500', 'text-white', 'hover:from-indigo-600', 'hover:to-purple-600');
        btn.classList.add('bg-white', 'text-black', 'hover:bg-gray-200');
    }
}

// 统一处理搜索或解析
function handleSearchOrParse() {
    const input = document.getElementById('searchInput');
    if (!input) return;
    const value = input.value.trim();
    if (isUrlInput(value)) {
...
function parseVideo(url) {
    ...
    window.location.href = 'parse.html?url=' + encodeURIComponent(url);
}
